...
async def main():
    ...
    await player.open(
        ...
        has_audio=False if richyplayer.IS_WEB else True,
    ...
    while running:
        ...
        if not player.busy_audio():
            player.play_audio(loops=1)
        player.set_frame(frameIndex)
        screen.blit(player.get_frame(), (0, 0))

        if not player2.busy_audio():
            player2.play_audio()
        player.set_frame(frameIndex)
        screen.blit(pygame.transform.scale_by(
            player2.get_frame(), 0.75), (500, 200))
